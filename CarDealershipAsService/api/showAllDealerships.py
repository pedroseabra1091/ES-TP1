...
def listDealerships():
	...
	for deals in session.query(Dealership.id, Dealership.name,Dealership.location, Owner.name).filter(Dealership.ownerID==Owner.id).order_by(Dealership.name).all():
		#print deals
