...
def create_Car():
	...
	session.query(Car).filter(Car.id == request.json['carID']).\
		update({Car.dealershipID : idDealership, Car.brand : request.json['brand'], Car.model : request.json['model'], Car.color : request.json['color'] , Car.plate : request.json['plate'], Car.mileage : request.json['mileage'], Car.fuelType : request.json['fuelType'],Car.year : request.json['year'],Car.price : request.json['price']},synchronize_session=False)
